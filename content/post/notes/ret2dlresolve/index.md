---
title: "ret2dlresolve"
description: "详解 ret2dlresolve 的延迟绑定原理与四种攻击手法，覆盖 No/Partial RELRO 下 32/64 位利用。"
date: 2026-08-14T15:32:33+08:00
slug: "ret2dlresolve"
image: ""
math: false
categories:
...
函数_dl_runtime_resolve(link_map，reloc_offset)可以对动态链接的函数进行重定位

第一次调用这个函数，先是到plt表，然后jmp到got表

![image-20260803170321249](/img/ref-ret2dlresolve/image-20250601205232833.png)

此时got表存的地址是在plt表上

![image-20260803170417129](/img/ref-ret2dlresolve/image-20250601205327472.png)
...
> 前面说的是延迟绑定，接下来_dl_fixup就是Linux动态链接器实现"延迟绑定"的真正执行者，它的任务是：根据传入的ID，查符号表和字符串表，找到函数的真实内存地址，把地址填回GOT表，然后把真实地址给CPU去执行
...
通过阅读_dl_fixup源码可以总结出一般的函数重定向流程：
...
在Linux中，程序使用_dl_runtime_resolve(link_map，reloc_offset)来对动态链接的函数进行重定位，对于动态链接在解析符号地址时所使用的重定位表项，动态符号表，动态字符串都是从目标文件的动态节.dynamic索引所得到的，所以如果可以修改其中的某些内容使得最后的动态链接器解析的符号是我们想要的解析的符号，那么攻击就达成了
...
![img](cd2fb2c3-6cc1-40a3-b91a-34196c703120.png)
...
5. **取函数名字符串：**
   ...
   **结果：** 指针第三次飞跃，落在了你写下的 "system\x00" 字符串上。
6. **终局：**
...
# 环境配置 
# context.log_level = "debug"        # 取消注释可以开启调试模式，打印所有收发的数据
context.terminal = ["tmux", "splitw", "-h"] # 设置 gdb 调试时使用的终端，这里用 tmux 切分窗口
context.arch = "i386"                # 指定目标程序架构为 32 位

#初始化
p = process("./no_relro_32")    # 启动目标程序
rop = ROP("./no_relro_32")      # 加载 ROP 模块，方便后续构造 ROP 链
elf = ELF("./no_relro_32")      # 解析 ELF 文件，方便获取节区（Section）数据和地址
...
# 获取 Shell 
p.interactive() # 移交控制权，允许用户与新开出来的 shell 交互
```
...
[深入理解ret2dlresolve | Collectcrop's Blog](https://collectcrop.github.io/blog/2025/06/02/深入理解ret2dlresolve/#no-relro-64)

![image-20260803170635369](/img/ref-ret2dlresolve/image-20250601210040967.png)

这题很明显，是一道栈溢出

![image-20260803170908724](/img/ref-ret2dlresolve/image-20250601210240496.png)
...
![image-20260803175647973](/img/ref-ret2dlresolve/image-20250601212109244.png)
...
![img](/img/ref-ret2dlresolve/image-20250601215505916.png)
...
r.interactive()                   # 享受你的 Root Shell 吧！
```
...
DT_STRTAB（指向 .dynstr 段的起始地址。这其实就是一个大号的字符数组，里面密密麻麻塞满了以 \x00 结尾的字符串（比如 "printf\x00puts\x00read\x00"））
